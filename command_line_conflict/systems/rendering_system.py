...
class RenderingSystem:
        ...
        if use_keys_iteration:
            ...
            for x, y in visible_keys:
                ...
                if entity_ids:
                    ...
                    self._draw_tile(entity_ids, cam_x, cam_y, game_state, paused, grid_size, bar_height, tile_size)
        else:
            # Fallback to grid iteration for dense maps or zoomed out views
            for y in range(start_y, end_y):
                # Optimization: Hoist Y calculation out of the inner loop
                cam_y = (y - self.camera.y) * tile_size
                for x in range(start_x, end_x):
                    # Only process if tile has entities
                    # Optimization: Get entity_ids directly to avoid double lookup (in check + get)
                    entity_ids = game_state.spatial_map.get((x, y))
                    if entity_ids:
                        cam_x = (x - self.camera.x) * tile_size
                        self._draw_tile(entity_ids, cam_x, cam_y, game_state, paused, grid_size, bar_height, tile_size)
    ...
    def draw_orders(self, components, tile_size: float) -> None:
        ...
        Args:
            ...
            tile_size: The size of a tile in pixels (grid_size * zoom).
        """
