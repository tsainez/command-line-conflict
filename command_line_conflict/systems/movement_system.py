    ...
    def update(self, game_state: GameState, dt: float) -> None:
        ...
        for entity_id in game_state.get_entities_with_component(Movable):
            ...
            if movable.path:
                ...
                # OPTIMIZATION: Use squared distance to avoid expensive math.sqrt when checking for arrival
                dist_sq = dx * dx + dy * dy
                if dist_sq < 0.0001:  # (0.01)^2
                    ...
                    # OPTIMIZATION: Calculate single step ratio instead of two separate divisions
