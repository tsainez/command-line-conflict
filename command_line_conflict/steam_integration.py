...
class SteamIntegration:
    ...
    def unlock_achievement(self, achievement_name: str) -> None:
        ...
        # Type check must happen first to avoid TypeError on len() for integers
        if not isinstance(achievement_name, str):
            log.warning(f"Security Warning: Invalid achievement name type: {type(achievement_name)}")
            return

        if not achievement_name or len(achievement_name) > 64:
            log.warning(f"Security Warning: Invalid achievement name length: {achievement_name}")
            return
