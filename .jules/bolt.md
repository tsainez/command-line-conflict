...
## 2024-06-20 - [Optimize Boolean Satisfaction Loops]
**Learning:** In ECS systems and game loops, iterating over all entities to count occurrences for boolean checks (like win/loss conditions) introduces significant O(N) overhead per frame.
**Action:** Replace total count accumulations with an early return (`return False/True`) inside the loop upon finding the first match. This converts an O(N) operation into an O(1) operation in the average case.
