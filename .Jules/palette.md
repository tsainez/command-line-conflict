...
## 2026-01-27 - Map Editor Toolbar Pattern
**Learning:** Replacing hidden hotkeys with visible, clickable toolbar buttons significantly improves discoverability for new users. However, removing static text instructions (like "Left Click: Toggle Wall") creates a regression where users know *how* to save/load but not *how* to edit the map.
**Action:** When adding interactive UI to replace static text, always ensure the core interaction instructions (mouse vs keyboard) are preserved, either in tooltips, status bars, or a dedicated "Help" button.
